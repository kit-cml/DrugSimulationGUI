...
QString ParameterViewer::get_simulation_directory()
{
    QString cell_name_val;
    int solver_type;
    for (int row = 0; row < model->rowCount(); ++row) {
        QString param_name = model->item(row, 0)->text();
        if (param_name == "cell_name") {
            QModelIndex index = model->index(row, 1);
            cell_name_val = model->data(index, Qt::EditRole).toString();
        }
        if (param_name == "solver_type") {
            QModelIndex index = model->index(row, 1);
            solver_type = model->data(index, Qt::EditRole).toInt();
        }
    }
    if( solver_type == 1 ){
        if(cell_name_val == "ord"){
            return cmlgui::directory::ORD_CVODE_DIRECTORY;
        }
        else if(cell_name_val == "ordstatic"){
            return cmlgui::directory::ORDSTATIC_CVODE_DIRECTORY;
        }
        else if(cell_name_val == "tomek"){
            return cmlgui::directory::TOMEK_CVODE_DIRECTORY;
        }
    }
    else{
        if(cell_name_val == "ord"){
            return cmlgui::directory::ORD_EULER_DIRECTORY;
        }
        else if(cell_name_val == "ordstatic"){
            return cmlgui::directory::ORDSTATIC_EULER_DIRECTORY;
        }
        else if(cell_name_val == "tomek"){
            return cmlgui::directory::TOMEK_EULER_DIRECTORY;
        }
    }

    return "SIMDIR_NOT_FOUND";
}
...
int ParameterViewer::get_sample_size()
{
    ...
    QDir::setCurrent(cmlgui::directory::DEFAULT_SIMULATION_ROOT+"/"+get_simulation_directory());
    ...
    qDebug() << "FILE FROM TABGLE: " << file_name;
    qDebug() << QDir::currentPath();
